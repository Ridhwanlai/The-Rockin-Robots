...
const int  LF_BASE_SPEED      = 100;    // Slower for better corner detection

// Hard turn: tight arc — fast wheel pulls, slow wheel barely creeps
const int  LF_HARD_FAST       = 145;
const int  LF_HARD_SLOW       = 2;

// Soft turn: gentle arc — both wheels move but clear difference
const int  LF_SOFT_FAST       = 100;
const int  LF_SOFT_SLOW       = 50;
...
const int           STUCK_COUNT_THRESHOLD = 3;
...
void lf_hardTurnLeft() {
  setMotors(LF_HARD_SLOW, LF_HARD_FAST);
}
void lf_hardTurnRight() {
  setMotors(LF_HARD_FAST, LF_HARD_SLOW);
}

void lf_softTurnLeft() {
  setMotors(LF_SOFT_SLOW, LF_SOFT_FAST);
}
void lf_softTurnRight() {
  setMotors(LF_SOFT_FAST, LF_SOFT_SLOW);
}
